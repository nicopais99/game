...
class Personaje_Enemigo(Objeto):
    ...
    def realizar_comportamiento(self, pantalla, lista_plataformas):
        ...
        for lados in self.lados:
            if self.lados["main"].right > LARGO -10:
                self.direccion_aleatoria = 0
...
class Boss(Personaje_Enemigo):
    ...
    def update(self, pantalla):
        self.animar(pantalla, diccionario_boss, "main")
        self.disparar()
        self.eliminar_boss()
    ...
    def disparar(self):
        ...
        proyectil_enemigo = Proyectil(x, y, "proyecto_pygame/proyectil/0.png", (20,30), diccionario_bullet_personaje, "Derecha")
        self.lista_disparos.append(proyectil_enemigo)

    def eliminar_boss(self):
        if self.vidas == 0:
            for lado in self.lados:
                self.lados[lado].x = 1000
                self.lados[lado].y = 1000
